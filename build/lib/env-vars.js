...
class EnvVars {
    ...
    constructor(vars, callback) {
        ...
        if (missingVariables.length > 0) {
            const err = new Error(`The following environment variables are not defined: ${missingVariables.join(', ')}`);
            if (callback) {
                callback(err);
                process.exit(1);
            }
     ...
     */
    getList(hide) {
        return [...this.requiredArray, ...this.optionalArray].map((variable) => `${variable} = ${hide && hide.indexOf(variable) !== -1
     ...
     */
    get(varName) {
        const value = [...this.requiredArray, ...this.optionalArray].find((name) => name === varName);
        if (!value) {
            throw new Error(`Can not get value of the environment variable "${varName}" because it vas not initialized by` +
